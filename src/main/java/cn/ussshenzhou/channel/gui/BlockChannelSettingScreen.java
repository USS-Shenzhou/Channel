...
public class BlockChannelSettingScreen extends TScreen {
    private final ChanneledBlockEntity channeledBlockEntity;
    private final TLabel title;
    private final TConstrainedEditBox channelEditBox;
    private TSlider channelSlider;
    private boolean updated = false;

    public BlockChannelSettingScreen(ChanneledBlockEntity channeledBlockEntity) {
        super(Component.literal("ChannelSettingScreen"));
        this.channeledBlockEntity = channeledBlockEntity;
        ...
        var channel = channeledBlockEntity.getChannel();
    ...
    public void tick() {
        if (updated) {
            var s = channelEditBox.getValue();
            if (channelEditBox.check(s)) {
                NetworkHelper.sendToServer(new SetBlockChannelPacket(channeledBlockEntity.getBlockPos(), getEditBoxValue()));
            }
            updated = false;
        }
        super.tick();
